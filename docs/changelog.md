...
**新增 EPUB 转换 Tab，支持批量转换并智能过滤非正文章节**

- 新增「EPUB 转换」Tab，独立于提取流程，专用于 EPUB → TXT 格式转换
- 前端纯 JS 实现，动态加载 JSZip 解压 EPUB，无需服务端支持
- 自动解析 EPUB OPF 元数据：书名、作者、出版年份、语言
- 智能过滤非正文章节：目录、引言/序言/推荐序、致谢、注释、附录、版权页、索引等
- 生成符合工作台格式要求的 TXT（含元数据头），可直接用于提取流程
- 支持批量上传、批量下载，实时进度显示（总进度 + 单文件进度）
- 解析完成后可编辑元数据（修正书名、作者、年份等），避免元数据错误导致后续判重问题
- 内容预览功能，可查看转换后的 TXT 格式和正文内容

**原因**：很多电子书资源只有 EPUB 格式，手动转换需要解压、提取 HTML、清理标签、整理格式，步骤繁琐。新增专用转换 Tab 后，批量拖拽上传即可自动完成解析和过滤，生成可直接用于提取的 TXT，大幅降低电子书预处理成本。

---

## v0.2.4 — 2026-04-05

**调试页面改为共享提取页面书籍数据，移除独立书表**

- 删除 `prompt_lab_books` 表，调试页面直接读取 `books` 表
- `prompt_lab_results.book_id` 外键改为引用 `books(id)`，级联删除
- 删除数据库函数：`upsertPromptLabBook`, `findPromptLabBookByMeta`, `listPromptLabBooks`, `getPromptLabBookById`, `deletePromptLabBookById`
- 调试页面移除上传功能，只保留选书、删除、清空调试结果
- 上传书籍统一在提取页面进行，避免数据重复管理

**原因**：之前调试页面和提取页面各自维护一套书籍数据，用户需要在两个地方重复上传。改为共享后，书籍只在提取页面上传一次，调试页面直接使用，减少数据冗余和操作成本。

---

## v0.2.3 — 2026-04-04

**新增调试页面（Prompt Lab），支持提取/审核 Prompt 快速迭代**

- 新增调试 Tab，独立于提取流程，用于 Prompt 实验室
- 支持自定义提取 Prompt 和审核 Prompt，实时查看 AI 输出
- 提取结果分块流式返回，边提取边审核
- 审核结果实时显示"通过/拒绝"判断及理由
- 调试数据存本地 SQLite `prompt_lab_books` 和 `prompt_lab_results` 表
- 支持多本书管理、清空调试结果、删除书籍

**原因**：提取 Prompt 和审核 Prompt 需要频繁调试和优化。如果每次都用完整提取流程验证，需要上传书籍、等待全部分块完成、审核大量候选句，迭代周期太长。新增调试页面后，可以用小段文本快速验证 Prompt 效果，确认后再到提取页面跑全书，大幅提升 Prompt 开发效率。
...
## v0.0.3 — 改用 Supabase Auth（Email+Password）
